...
public class EmployeeController {
    ...
    public @ResponseBody ResponseEntity<String> addEmployee(@RequestBody Employee employee) {
        ...
        LocalDate birthdate = Instant.ofEpochMilli(employee.getBirthdate().getTime()).atZone(ZoneId.systemDefault())
                .toLocalDate();

        Period period = Period.between(birthdate, now);

        if (employeeService.findIdEmployee(employee.getName(), employee.getLast_name()).size() > 0
                || (period.getYears() < 18 || !jobsService.findJobById(employee.getJob_id()).isPresent())) {
            success = false;
